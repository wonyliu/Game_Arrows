...
export class UI {
    ...
    async renderLeaderboard() {
        ...
        const renderFallback = (message) => {
            ...
            if (this.leaderboardEmptyStateEl) {
                this.leaderboardEmptyStateEl.textContent = message || this.getLocaleText('闁圭儤甯熼、鎴濐潡濠婂嫭鐣☉鎾崇Т瑜版煡鎮介妸锝傚亾?, 'Leaderboard unavailable.');
            }
        };
        ...
        try {
            ...
            if (!response.ok || rows.length === 0) {
                renderFallback(this.getLocaleText('闁哄棗鍊瑰Λ銈夊箳閹烘洦鏀芥慨鎺撶矋閺嗙喖骞戦琛″亾?, 'No leaderboard data yet.'));
                return;
            }
            ...
            if (this.leaderboardEmptyStateEl) {
                this.leaderboardEmptyStateEl.textContent = this.getLocaleText(
                    '闁圭顦板〒鑸殿殗濮椻偓閳ь剚鑹鹃崣褔宕楅崘鎻掑耿闁圭儤甯熼、鎴︽晬鐏炶偐鐭岄柡鍕⒔閵囨岸宕?100 闁告艾绉查埀?,
                    'Sorted by highest cleared level. Top 100 only.'
                );
            }
        } catch {
            renderFallback(this.getLocaleText('闁圭儤甯熼、鎴濐潡濠婂啫顫ｉ弶鐐舵閵囨垹鎷归妷顖滅閻犲洭顥撻埣銏ゅ触鎼淬劌娅㈤悹鍥ㄦ磸閳?, 'Failed to load leaderboard.'));
        }
    }
    ...
    buildLeaderboardRow(row, selfUserId = '') {
        ...
        if (isSelf) {
            ...
            selfTagEl.textContent = this.getLocaleText('闁挎稑鐗婇崹婊堟晬?, '(Me)');
        ...
        levelEl.textContent = level > 0
            ? this.getLocaleText(`闂侇偅鑹鹃崣?${level}`, `Clear ${level}`)
            : this.getLocaleText('闁哄牜浜埀顒佽壘閸?, 'Not Cleared');
    ...
    renderLeaderboardSelfRow(row, selfUserId = '') {
        ...
        if (this.leaderboardSelfLabelEl) {
            this.leaderboardSelfLabelEl.textContent = this.getLocaleText('闁瑰瓨鍨瑰▓鎴﹀箳閹烘垶鍊?, 'My Rank');
        }
    ...
    showCheckinRewardTooltip(anchorEl, day, rewards = [], pointerEvent = null) {
        ...
        const title = this.locale === 'zh-CN' ? `缂?{day}濠㈠灈鏅涢〃娑㈠礉閻?: `Day ${day} Reward`;
    ...
    createCheckinRewardNode(day, rewards, isClaimableDay, isClaimedDay) {
        ...
        node.setAttribute('aria-label', this.locale === 'zh-CN'
            ? `缂?{day}濠㈠灈鏅濋鐑藉礆閺夋娈柛鏃€鍞荤槐?{this.formatRewardList(rewards)}`
            : `Day ${day} check-in reward: ${this.formatRewardList(rewards)}`);

        const title = document.createElement('div');
        title.className = 'checkin-day-title';
        title.textContent = this.locale === 'zh-CN' ? `缂?{day}濠㈣翰鍏?: `Day ${day}`;
    ...
    refreshCheckinPanel() {
        ...
        if (this.checkinStatusEl) {
            this.checkinStatusEl.textContent = checkin.canClaimToday
                ? (this.locale === 'zh-CN' ? `闁绘劗鎳撻崵顔剧箔?{checkin.nextDayIndex}濠㈠灈鏅涢〃娑㈠礉閸楃偛骞㈤柛妤€鍟胯ぐ鍙夛紣閸℃绲縛 : `Tap day ${checkin.nextDayIndex} card to claim`)
    ...
    refreshOnlineRewardDock() {
        ...
        if (!online.enabled) {
            this.onlineDockTextEl.textContent = this.locale === 'zh-CN' ? '闁哄牜浜滅槐鎴﹀触? : 'Off';
            return;
        }
        if (online.done) {
            this.onlineDockTextEl.textContent = this.locale === 'zh-CN' ? '鐎瑰憡鐓￠。顐も偓? : 'Done';
            return;
        }
        if (online.canClaim) {
            this.onlineDockTextEl.textContent = this.locale === 'zh-CN' ? '闁告瑯鍨堕。顐﹀矗? : 'Claim';
            return;
        }
    ...
    showCheckinRewardSettle(payload = {}) {
        ...
        if (this.checkinRewardSettleDescEl) {
            this.checkinRewardSettleDescEl.textContent = this.locale === 'zh-CN'
                ? '缂佹稒鍎抽崺宀勫箣閹邦剙顫犻柨娑樿嫰椤ㄦ盯宕濋崡鐑嗘搐濞戞挸顑戠槐?
        ...
        if (this.btnCheckinRewardConfirm) {
            ...
            this.btnCheckinRewardConfirm.textContent = this.locale === 'zh-CN' ? '缁绢収鍠栭悾? : 'Confirm';
        }
    ...
    showOnlineRewardSettle(payload = {}) {
        ...
        if (this.onlineRewardSettleDescEl) {
            this.onlineRewardSettleDescEl.textContent = this.locale === 'zh-CN' ? '闁哄牜鍓氶濂稿捶閵娧冩疇濠靛倹鐗曟慨鎶芥晬? : 'Online reward:';
        }
        ...
        if (this.btnOnlineRewardSettleClose) {
            ...
            this.btnOnlineRewardSettleClose.textContent = this.locale === 'zh-CN' ? '缁绢収鍠栭悾? : 'Confirm';
        }
    ...
    showRewardUnlockToast(payload = {}) {
        ...
        if (textEl) {
            textEl.textContent = threshold > 0
                ? `${threshold} 閺夆晝鍋涢崵顔炬喆閿曗偓瑜板倹绺介弽褍小闁稿繒鐗?
                : '闁谎勫礃缁绘盯寮埡渚囨闁告柨宕崣?;
    ...
    showLevelCompletePopup() {
        ...
        if (this.levelCompleteNextButton) {
            this.levelCompleteNextButton.textContent = willEnterRewardStage ? '濠靛倹鐗曟慨鎶藉礂? : '濞戞挸顑勭粩鎾礂?;
        }
        if (this.levelCompleteTitleEl) {
            this.levelCompleteTitleEl.textContent = isCampaignComplete ? '闁诡収鍘奸弸鈺呮焻濮橆剙褰? : '闁诡収鍘奸弸鈺傛交閸パ冨綘';
        }
